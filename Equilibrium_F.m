...
alpha2 = 0; % Repulsion strength between crowd and shooter
alpha3 = -1; % Attraction strength toward guiding agent
...
z0 = [1.5; 0.5]; % Initial position of shooter
...
for i = 1:num_steps
    ...
    k1 = motion(x(:,:,i), y(:,i), z(:,i), N, r0, alpha1, alpha2,alpha3);
    k2 =  motion(x(:,:,i)+ 0.5*dt*k1(:,1:N), y(:,i)+ 0.5*dt*k1(:,N+1), z(:,i)+ 0.5*dt*k1(:,N+2), N, r0, alpha1, alpha2,alpha3);
    k3 =  motion(x(:,:,i)+ 0.5*dt*k2(:,1:N), y(:,i)+ 0.5*dt*k2(:,N+1), z(:,i)+ 0.5*dt*k2(:,N+2), N, r0, alpha1, alpha2,alpha3);
    k4 =  motion(x(:,:,i)+ 0.5*dt*k3(:,1:N), y(:,i)+ 0.5*dt*k3(:,N+1), z(:,i)+ 0.5*dt*k3(:,N+2), N, r0, alpha1, alpha2,alpha3);
...
plot(y(1, end), y(2, end), 'g+', 'MarkerSize', 30, 'LineWidth', 30);
plot(z(1, end), z(2, end), 'rx', 'MarkerSize', 30, 'LineWidth', 30);
set(gca,'color', 'w','FontName','times','Fontsize',17);
axis equal
axis off
saveas(fig,'caseF.eps')
saveas(fig,'caseF.fig')
saveas(fig,'caseF.jpg')

function dX = motion(x, y, z, N, r0, alpha1, alpha2,alpha3)
dx = zeros(size(x));
for j = 1:N
    for k = 1:N
        if k ~= j
